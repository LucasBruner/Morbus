...
@DisplayName("TransicionarParaAtendidaUseCase")
class TransicionarParaAtendidaUseCaseTest {

    @Mock
    private ISolicitacaoRepository repository;

    private TransicionarParaAtendidaUseCase useCase;

    @BeforeEach
    void setUp() {
        useCase = new TransicionarParaAtendidaUseCase(repository);
    }
    ...
    class QuandoAgendada {
        ...
        @Test
        @DisplayName("deve transicionar o status para ATENDIDA")
        void deveTransicionarParaAtendida() {
            ...
            useCase.execute(new AppointmentAttendedCommand(solicitacao.getId(), UUID.randomUUID(), LocalDateTime.now()));

            assertThat(solicitacao.getStatus()).isEqualTo(EStatusSolicitacao.ATENDIDA);
        }
    ...
    class QuandoNaoExiste {
        ...
        void naoDeveLancarExcecao() {
            ...
            useCase.execute(new AppointmentAttendedCommand(solicitacaoId, UUID.randomUUID(), LocalDateTime.now()));
        }
    ...
    @Nested
    @DisplayName("quando a solicitacao ja esta ATENDIDA (idempotencia)")
    class QuandoJaAtendida {

        @Test
        @DisplayName("nao deve lancar excecao")
        void naoDeveLancarExcecao() {
            Solicitacao solicitacao = buildSolicitacao(EStatusSolicitacao.ATENDIDA);
            when(repository.findById(solicitacao.getId())).thenReturn(solicitacao);
    ...
    class QuandoStatusNaoPermitido {
        ...
        @EnumSource(value = EStatusSolicitacao.class,
                names = {"AGUARDANDO", "APROVADA", "NEGADA", "CANCELADA", "DEVOLVIDA", "PENDENTE", "FALTOU"})
