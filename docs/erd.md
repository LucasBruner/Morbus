...
> **Ordenação real da fila — correção importante:** `PriorityCalculator.compare()` (o comparador de 4 critérios documentado em `logica_priorizacao_sus.md`) **não é chamado em nenhum fluxo do sistema** — é código morto, sem callers. A ordenação de fato acontece em SQL, via `QueueEntryJpaRepository` (`findByPriority`/`findAllOrderedByPriority`/`findByProcedureIdAndFilters`):
> `ORDER BY CASE tipo_fila WHEN 'FILA_REGULADA' THEN 0 ELSE 1 END, risk_color, patients.grupo_legal, registered_at`.
> A precedência `FILA_REGULADA` > `FILA_ESPERA` é real, mas a ordenação usa a coluna **estática** `patients.grupo_legal` — não um recálculo dinâmico de idade. Um paciente que completa 60 anos **enquanto aguarda na fila** só é promovido a `IDOSO` se seu cadastro for atualizado (`PATCH /api/v1/patients/{id}`); a simples passagem do tempo não o promove automaticamente na ordenação. `PriorityCalculator.getPriorityGroup()`/`isPriorityGroup()` só são usados como *snapshot* em `RegisterPatient`, `UpdatePatient` e `RegisterPatientInQueue`.
